...
func (rt *_router) Handler() http.Handler {
	...
	rt.router.POST("/users/:name/bans/", rt.wrap(banUser))              // works
	rt.router.GET("/stream", rt.wrap(getMyStream))                      // works
	rt.router.PATCH("/users/:name", rt.wrap(setMyUserName))             // works
	rt.router.DELETE("/users/:name/bans/", rt.wrap(unbanUser))          // works
	...
	rt.router.GET("/photos/:photoId/comments/", rt.wrap(getComments))   // about to work
